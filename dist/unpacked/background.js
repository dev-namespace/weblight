...
const commands = {
  ...
  removeHighlight: data => {
    sendPOST('http://www.weblight.com:3000/hl/remove', {
      id: data.id
    });
  },
  searchHighlights: (data, callback) => {
    sendPOST('http://www.weblight.com:3000/hl/search', {
      query: data
    }, response => {
      callback(response || {
        highlights: []
  ...
  queryHighlights: (data, callback) => {
    sendPOST('http://www.weblight.com:3000/hl/query', {
      query: data
    }, response => {
      callback(response || {
        highlights: []
...
function callbackMsg(cid, args) {
  chrome.tabs.query({
    ...
    const msg = {
      type: 'callback',
      cid,
      args // console.log('callback msg', msg)
