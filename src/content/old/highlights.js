...
export async function restoreHighlights() {
    if(document.readyState === 'complete') {
        //@TODO merge local highlights in
...
//@TODO sync localStorage highlights
// sync local deletions?
// collection object what fetchHighlights return
// 1. Login
//   - if server offline -> offline mode
//     - retrieve local collection (if user exists)
//     - log pending operations (delete id, create id) <- delete only if exists!
//   - if server online
//     - create local user if doesn't exist
//     - apply pending operations (if last change on server is more recent, ask for confirmation)
//     - fetch updated collection (fetchHighlights)
//     - override local collection (truncate on size)
//     - store every change in localStorage and server
